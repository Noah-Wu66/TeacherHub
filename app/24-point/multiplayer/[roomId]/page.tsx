    ...
    <div className="min-h-dvh flex flex-col items-center px-3 sm:px-4 py-2 sm:py-6">
      ...
      <div className="w-full max-w-3xl flex items-center justify-between mb-3 sm:mb-6">
        <button
          ...
          className="flex items-center gap-1 text-sm text-emerald-100/60 hover:text-white active:text-emerald-50 transition-colors cursor-pointer min-h-[44px] min-w-[44px] px-1"
        >
        ...
        <h1 className="text-base sm:text-lg md:text-xl font-bold text-white drop-shadow-sm">
          {room?.roomName || '对战房间'}
        </h1>
        <div className="text-xs text-emerald-100/40 font-mono min-w-[44px] text-right">{roomId}</div>
      </div>

      {/* 加载/错误 */}
      {loading && !room && (
        <div className="flex-1 flex items-center justify-center">
          <p className="text-emerald-100/60">加载中...</p>
        </div>
      )}
