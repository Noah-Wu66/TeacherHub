      ...
      <div className="w-full max-w-lg md:max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
        {modes.map((mode, index) => (
          <Link key={mode.href} href={mode.href} className="block w-full outline-none">
            ...
            >
              {/* 卡片内置流光彩晕 */}
              <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${mode.gradient} opacity-10 blur-[40px] group-hover:opacity-30 transition-opacity duration-300`} />
              <div className={`absolute bottom-0 left-0 w-24 h-24 bg-gradient-to-tr ${mode.gradient} opacity-5 blur-[30px] group-hover:opacity-20 transition-opacity duration-300`} />
