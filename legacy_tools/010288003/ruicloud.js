...
function playShortSound(type) {
    ...
    shortAudio.play();
    setTimeout(function () {
        shortAudio.parentNode.removeChild(shortAudio);
    }, 2000);
}
...
function showFinishAlert(type, duration, text, fontSize, color, top) {
    var audio = new Audio();
    console.log('showFinishAlert audio ogg ' + audio.canPlayType('audio/ogg'));
    console.log('showFinishAlert audio mp3 ' + audio.canPlayType('audio/mpeg'));
    let soundType = 0;
    if (audio.canPlayType('audio/ogg') != '') {
        soundType = 0;
    } else if (audio.canPlayType('audio/mpeg') != '') {
